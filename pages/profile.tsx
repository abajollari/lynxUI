...
import { completeRegistration } from 'store/actions/auth'
...
export default function Profile() {
    ...
    async function getProfileInfo() {
        ...
        const response: AxiosResponse = await apiClient.getProfileInfo(
            userEmail
        )
        switch (response.status) {
            case 200:
                //const date = new Date(response.data.birthday)
                //const bd = `${date.getFullYear()}-${('0' + date.getMonth()).slice(-2 )}-${('0' + date.getDate()).slice(-2)}`

                setFirstName(response.data.firstName)
                setMiddleName(response.data.middleName)
                setLastName(response.data.lastName)
                setBirthday(response.data.birthday) //bd
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        var dt = Date.parse(birthday);
        if(isNaN(dt))
        {
            store.dispatch(
                pushActionMessage('Please enter a valid birthday date!')
            )
            return
        }
        ...
        if (
            ...
            try {
                ...
                const response = await apiClient.sendKycData(kycRequset)
                store.dispatch(
                    pushActionMessage(
                ...
                store.dispatch(completeRegistration(response.data.firstName))
    ...
    return (
        <div className={style.container}>
            ...
            >
                <div className={`${style.formContainer} ${style.span2}`}>
                    <h1 className={style.headTitle}>Profile</h1>
                    <Input
                        type="text"
